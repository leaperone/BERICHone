...
export default function Footer() {
  const renderList = React.useCallback(
    ({ title, items }: { title: string; items: { name: string; href: string }[] }) => (
      <div>
        <h3 className="font-semibold text-default-600 text-small">{title}</h3>
      ...
      <div className="px-6 pt-16 pb-8 sm:pt-24 lg:px-8 lg:pt-32">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          <div className="space-y-8 md:pr-8">
            ...
            <p className="text-default-500 text-small">People, Live Streaming, Life</p>
            {/* TODO: social links */}
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-8">
          <p className="text-default-400 text-small">&copy; 2025 LEAPERone Inc. All rights reserved.</p>
          {beiAnLinks.map((link) => (
            <Link key={link.label} href={link.to} size="sm" className="text-default-400">
              {link.label}
            </Link>
          ))}
